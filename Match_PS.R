#script for proposed adjustment under full-matching 
unlink(".RData*")
unlink(".RData")
library(xtable)
library(maxLik)
library(MatchIt)
library(twang)
if(file.exists(paste(getwd(), "/results/MatchPS",s,".RData", sep=""))==FALSE) 
{
set.seed(12498+s)
...
#adjusting only by matching
#model 1,y~trt+c, ps_true
countF<-9
countL<-countF+7
count<-1
mm1<-count+1
data2<-match.data((matchit(trt~c1+c2+c3+c4+c5+c6, data=data1, method="full")))
...
betas[m,countF:countL]<-glm(y~trt+c1+c2+c3+c4+c5+c6, family=binomial(logit), data=data2, weights=w)$coefficients
...
countL<-countF+7
mm1<-mm1+1
data2<-match.data((matchit(trt~c1+c2+c3+c4+c5+c6, data=data1, distance=data1$ps_ep,  method="full")))
data2$w<-data2$weights
betas[m,countF:countL]<-glm(y~trt+c1+c2+c3+c4+c5+c6, family=binomial(logit), data=data2, weights=w)$coefficients
...
countL<-countF+7
mm1<-mm1+1
data2<-match.data((matchit(trt~c1+c2+c3+c4+c5+c6, data=data1, distance=data1$ps_adj, method="full")))
data2$w<-data2$weights
betas[m,countF:countL]<-glm(y~trt+c1+c2+c3+c4+c5+c6, family=binomial(logit), data=data2, weights=w)$coefficients
...
data2<-match.data((matchit(trte~c1+c2+c3+c4+c5+c6, data=data1, distance=data1$ps_true, method="full")))
data2$w<-data2$weights
betas[m,countF:countL]<-glm(y~trte+c1+c2+c3+c4+c5+c6, family=binomial(logit), data=data2, weights=w)$coefficients
...
countL<-countF+7
mm1<-mm1+1
data2<-match.data((matchit(trte~c1+c2+c3+c4+c5+c6, data=data1, method="full")))
data2$w<-data2$weights
betas[m,countF:countL]<-glm(y~trte+c1+c2+c3+c4+c5+c6, family=binomial(logit), data=data2, weights=w)$coefficients
...
data2<-match.data((matchit(trte~c1+c2+c3+c4+c5+c6, data=data1, distance=data1$ps_adj, method="full")))
data2$w<-data2$weights
betas[m,countF:countL]<-glm(y~trte+c1+c2+c3+c4+c5+c6, family=binomial(logit), data=data2, weights=w)$coefficients
...
betas[m,countF:(countF+1)]<-glm(y~trt, family=binomial(logit), data=data2, weights=w)$coefficients
...
betas[m,countF:(countF+1)]<-glm(y~trte, family=binomial(logit), data=data2, weights=w)$coefficients
betas[m,(countF+2):countL]<-'NA'
#marginal beta
a<-1/(1+exp(-as.numeric(betas[m,countF])-as.numeric(betas[m,(countF+1)])))
b<-1/(1+exp(-as.numeric(betas[m,countF])))
...
data2<-match.data((matchit(trte~c1+c2+c3+c4+c5+c6, distance=data1$ps_true, data=data1, method="full")))
data2$w<-data2$weights
dataT1<-data1
dataT2<-data_val
data1<-data2
data2val<-match.data((matchit(trt~c1+c2+c3+c4+c5+c6, data=data_val, method="full")))
data2val$w<-data2val$weights
...
save(betas, betas_marg, file = paste(getwd(),"/results/MatchPS",s,".RData", sep=""))
}